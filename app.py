    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            try:
                ...
                db.session.rollback()
                echeck = db.session.query(exists().where(User.email == form.email.data)).scalar()
                ucheck = db.session.query(exists().where(User.username == form.username.data)).scalar()
                if echeck==True:
...
def reset_email():
    ...
    if form.validate_on_submit():
        try:
            ...
            flash(f"Email Address {form.email.data} Doesn't match our record", 'error')
...
def token_reset(token):
    ...
    if form.validate_on_submit():
        try:
            ...
            flash(f"Email Address {email} Doesn't match our record", 'error')
            return redirect(url_for('login'))
